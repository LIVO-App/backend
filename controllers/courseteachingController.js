...
module.exports.add_single_teaching = async (req, res) => {
    if(req.loggedUser.role==="admin"){
        ...
        if(!user_exist){
            ...
            console.log('course single teaching insert: unauthorized access');
            return;
        }
    } else {
        res.status(401).json({status: "error", description: MSG.notAuthorized});
    ...
    if(!course_exist){
        res.status(404).json({status: "error", description: MSG.notFound});
        console.log('course single teaching insert: course not found');
        return;
    }
    ...
    if(!teaching_exist){
        res.status(404).json({status: "error", description: MSG.notFound});
        console.log('course single teaching insert: teaching not found');
        return;
    }
    ...
    if(teaching_entry_exist){
        ...
        console.log('course single teaching insert: duplicate entry');
        return;
    }
...
module.exports.delete_teaching = async (req, res) => {
    if(req.loggedUser.role==="admin"){
        ...
        if(!user_exist){
            res.status(401).json({status: "error", description: MSG.notAuthorized});
            console.log('course single teaching remove: unauthorized access');
            return;
        }
    } else {
        res.status(401).json({status: "error", description: MSG.notAuthorized});
    ...
    if(!course_exist){
        res.status(404).json({status: "error", description: MSG.notFound});
        console.log('course single teaching remove: course not found');
        return;
    }
    let teaching_id = req.query.teaching_id;
    let teaching_exist = await teachingModel.read(teaching_id);
    if(!teaching_exist){
        res.status(404).json({status: "error", description: MSG.notFound});
        console.log('course single teaching remove: teaching not found');
        return;
    }
