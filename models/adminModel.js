...
async function read(condition,param){
    try {
        ...
        sql = "SELECT id, cf, username, name, surname, gender, birth_date, address, email, google, first_access, assets FROM admin WHERE " + condition;
...
module.exports = {
    ...
    async list() {
        try{
            conn = await pool.getConnection();
            sql = `SELECT cf, username, name, surname, gender, birth_date, address FROM admin ORDER BY surname`;
            //console.log(sql);
    ...
    async add_admin(cf, username, email, psw, name, surname, gender, birth_date, address, assets_link, google = false){
        try {
            conn = await pool.getConnection()
            ...
            let sql = 'INSERT INTO admin (cf, username, email, `password`, name, surname, gender, birth_date, address, google, first_access, assets) VALUES (?,?,?,?,?,?,?,?,?,?, 1,?)'
