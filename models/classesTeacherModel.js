...
module.exports = {
    ...
    async read_project_classes_teach(teacher_id, session_id, course_id){
        ...
        try {
            ...
            let sql = "SELECT c.id, CASE WHEN pc.italian_displayed_name IS NULL THEN c.italian_title ELSE pc.italian_displayed_name END AS 'italian_title', CASE WHEN pc.english_displayed_name IS NULL THEN c.english_title ELSE pc.english_displayed_name END AS 'english_title', pt.section, ass.teaching_id, CASE WHEN ass.teaching_id IN (SELECT ot.teaching_id FROM ordinary_teach AS ot WHERE ot.teacher_id=?) THEN 1 ELSE 0 END AS my_teaching FROM course AS c JOIN project_teach AS pt ON c.id = pt.project_class_course_id JOIN associated AS ass ON ass.course_id = c.id JOIN project_class AS pc ON pt.project_class_course_id = pc.course_id AND pt.project_class_session = pc.learning_session_id WHERE pt.teacher_id = ? AND pt.project_class_session = ?";
    ...
    async read_project_classes_associated(teacher_id, session_id, course_id){
        ...
        try {
            ...
            let sql = "SELECT DISTINCT c.id, CASE WHEN pc.italian_displayed_name IS NULL THEN c.italian_title ELSE pc.italian_displayed_name END AS 'italian_title', CASE WHEN pc.english_displayed_name IS NULL THEN c.english_title ELSE pc.english_displayed_name END AS 'english_title', ins.section, ass.teaching_id FROM course AS c JOIN project_class AS pc ON c.id = pc.course_id JOIN inscribed AS ins ON pc.course_id = ins.project_class_course_id AND pc.learning_session_id = ins.project_class_session JOIN associated AS ass ON c.id = ass.course_id WHERE ins.student_id IN (SELECT s.id FROM student AS s JOIN attend AS att ON s.id = att.student_id WHERE att.ordinary_class_study_year IN (SELECT ot.ordinary_class_study_year FROM ordinary_teach AS ot WHERE ot.teacher_id = ?) AND att.ordinary_class_address IN (SELECT ot.ordinary_class_address FROM ordinary_teach AS ot WHERE ot.teacher_id = ?)) AND pc.learning_session_id = ? AND ass.teaching_id IN (SELECT ot.teaching_id FROM ordinary_teach AS ot WHERE ot.teacher_id = ?)";
    ...
    async update(course_id, session_id, teacher_list){
        try {
            conn = await pool.getConnection()
            if(!course_id || !session_id || teacher_list == undefined ||teacher_list.length == 0){
                conn.release()
                return false
            }
            let sql = ''
            ...
            for(let i=0;i<teacher_list.length;i++){
                ...
                let sections = teacher_list[i]["sections"]
                for(let j=0;j<sections.length;j++){
                    sql += 'UPDATE project_teach SET main = ? WHERE teacher_id = ? AND project_class_course_id = ? AND project_class_session = ? AND section = ?';
                    values.push(main_teacher, teacher_id, course_id, session_id, sections[j])
                    if(j<sections.length-1){
                        sql+='; '
                    }
                }
