...
module.exports = {
    async list(course_id, block_id, section, publisher_id, is_admin = false, is_student=false){
        try {
            ...
            if(is_student){
                sql += 'JOIN project_class AS pc ON pc.course_id = ann.project_class_course_id AND pc.learning_block_id = ann.project_class_block '
            }
            sql += 'WHERE ann.project_class_course_id = ? AND ann.project_class_block = ? AND ann.section = ?';
    ...
    async add(publisher_id, is_admin = false, course_id, block_id, section, italian_title, english_title, italian_message, english_message, publish_date){
        try{
            conn = await pool.getConnection();
