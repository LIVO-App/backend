...
module.exports = {
    ...
    async add_single(course_id, teaching_id){
        try {
            let conn = await pool.getConnection()
            if(course_id==undefined || teaching_id==undefined){
                conn.release()
                return false
            }
            let sql = 'INSERT INTO associated (course_id, teaching_id) VALUES  (?,?)';
            ...
            console.log("Something went wrong: insert course single teaching")
        } finally {
            conn.release()
        }
    },
    async remove(course_id, teaching_id){
        try {
            let conn = await pool.getConnection()
            if(course_id==undefined || teaching_id==undefined){
                conn.release()
            ...
            let sql = 'DELETE FROM associated WHERE course_id = ? AND teaching_id = ?';
            let values = [course_id, teaching_id]
            const rows = await conn.query(sql, values)
            conn.release()
            return rows
